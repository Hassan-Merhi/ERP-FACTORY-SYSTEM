...
describe("historical replay timeline", () => {
  ...
  it("flags quantity drift and only clamps tiny residuals", async () => {
    const tiny = await replaySupplierTimeline(7, 3, "Supplier", 0, [
      { kind: "RECEIPT", effectiveDate: "2026-01-01", createdAt: 1, stableId: 1, receiptKg: 10, canonicalRateUsd: 2 },
      { kind: "BATCH_CONSUMPTION", effectiveDate: "2026-01-02", createdAt: 2, stableId: 2, batchId: 1, consumptionKg: 10.0005 },
    ], 0);
    const real = await replaySupplierTimeline(7, 3, "Supplier", 0, [
      { kind: "RECEIPT", effectiveDate: "2026-01-01", createdAt: 1, stableId: 1, receiptKg: 10, canonicalRateUsd: 2 },
      { kind: "BATCH_CONSUMPTION", effectiveDate: "2026-01-02", createdAt: 2, stableId: 2, batchId: 1, consumptionKg: 11 },
    ], 0);
...
describe("historical replay event builders", () => {
  it("builds receipt history and falls back to active stock when no receipt rows exist", async () => {
    const activeRawStock = { containerId: 10, receivedKg: "80", offloadedAt: "2026-01-04T00:00:00.000Z" } as any;
    const canonicals: CanonicalContainer[] = [
      {
        universe: universe({ activeRawStock }),
      ...
      },
      {
        ...
        canonicalTotalUsd: 40,
        storedCostPerKgUsd: 4,
        storedTotalUsd: 40,
        fxUnresolved: true,
        safeToRepair: false,
      ...
      },
      {
        ...
        canonicalTotalUsd: 50,
        storedCostPerKgUsd: 5,
        storedTotalUsd: 50,
        fxUnresolved: false,
        safeToRepair: true,
        reason: null,
    ...
    const events = await buildReceiptEvents(executor([[
      { id: 99, container_id: 10, receipt_date: "2026-01-05", received_kg: "25", created_at: "2026-01-05T00:00:00Z" },
    ]]), 7, canonicals);

    expect(events).toHaveLength(2);
    expect(events[0]).toMatchObject({ containerId: 10, supplierId: 3, receiptKg: 25, stableId: 99 });
    expect(events[1]).toMatchObject({ containerId: 12, receiptKg: 100, stableId: -12 });
  });

  it("classifies adjustment valuation and counts unclassified valued additions", async () => {
    const result = await buildAdjustmentEvents(executor([[
      { id: 1, supplier_id: 3, date: "2026-01-01", type: "ADD", kg: "5", cost_per_kg: "2", currency_code: "USD", valuation_basis: null },
      { id: 2, supplier_id: 3, date: "2026-01-02", type: "ADD", kg: "4", cost_per_kg: "7", currency_code: "EUR", valuation_basis: "VALUED_TRANSFER" },
      { id: 3, supplier_id: 3, date: "2026-01-03", type: "REMOVE", kg: "2", cost_per_kg: "0", currency_code: "USD", valuation_basis: null },
      { id: 4, supplier_id: 4, date: null, type: "DEDUCT", kg: "1", cost_per_kg: "0", currency_code: "USD", valuation_basis: "QUANTITY_ONLY" },
    ]]), 7);
  ...
  it("resolves source ownership, skips non-consuming bases, and aggregates supplier consumption", async () => {
    const result = await buildBatchConsumptionEvents(
      executor([
        [{ id: 20, batch_code: "B20", batch_date: "2026-01-05", status: "OPEN", created_at: "2026-01-05", cost_per_kg: "3", total_cost: "30", total_weight_kg: "10" }],
        [
          { id: 1, mix_batch_id: 20, supplier_id: 3, container_id: null, source_batch_id: null, weight_kg: "4", cost_per_kg: "2", total_cost: "8", inventory_supplier_id: null },
          { id: 2, mix_batch_id: 20, supplier_id: null, container_id: 10, source_batch_id: null, weight_kg: "3", cost_per_kg: "4", total_cost: "12", inventory_supplier_id: null },
          { id: 3, mix_batch_id: 20, supplier_id: null, container_id: null, source_batch_id: 19, weight_kg: "2", cost_per_kg: "1", total_cost: "2", inventory_supplier_id: null },
          { id: 4, mix_batch_id: 20, supplier_id: 3, container_id: null, source_batch_id: null, weight_kg: "1", cost_per_kg: "2", total_cost: "2", inventory_supplier_id: null },
          { id: 5, mix_batch_id: 20, supplier_id: 99, container_id: null, source_batch_id: null, weight_kg: "1", cost_per_kg: "2", total_cost: "2", inventory_supplier_id: null },
        ],
        [{ id: 10, supplier_id: 3 }],
      ]),
      7,
...
describe("historical replay universe read model", () => {
  it("selects the strongest scan reason and keeps the newest active raw-stock row", async () => {
    const result = await loadContainerUniverse(executor([
      [
        { id: 10, supplier_name: "S", has_active_rs: true, has_deleted_rs: true, has_receipt_history: true, has_offload_daybook: true, has_mix_source: true, earliest_offload_date: "2026-01-02", offloaded_at: null },
        { id: 11, supplier_name: "S", has_active_rs: false, has_deleted_rs: true, has_receipt_history: false, has_offload_daybook: false, has_mix_source: false, earliest_offload_date: null, offloaded_at: "2026-01-03" },
        { id: 12, supplier_name: "S", has_active_rs: false, has_deleted_rs: false, has_receipt_history: true, has_offload_daybook: false, has_mix_source: false, earliest_offload_date: null, offloaded_at: null },
        { id: 13, supplier_name: "S", has_active_rs: false, has_deleted_rs: false, has_receipt_history: false, has_offload_daybook: true, has_mix_source: false, earliest_offload_date: "2026-01-04", offloaded_at: null },
        { id: 14, supplier_name: "S", has_active_rs: false, has_deleted_rs: false, has_receipt_history: false, has_offload_daybook: false, has_mix_source: true, earliest_offload_date: null, offloaded_at: null },
      ],
      [
        { id: 10, company_id: 7, container_id: 10, actual_received_kg: "100", rate_per_kg: "2", rate_per_kg_usd: "2" },
        { id: 11, company_id: 7, container_id: 11, actual_received_kg: "100", rate_per_kg: "2", rate_per_kg_usd: "2" },
        { id: 12, company_id: 7, container_id: 12, actual_received_kg: "100", rate_per_kg: "2", rate_per_kg_usd: "2" },
        { id: 13, company_id: 7, container_id: 13, actual_received_kg: "100", rate_per_kg: "2", rate_per_kg_usd: "2" },
        { id: 14, company_id: 7, container_id: 14, actual_received_kg: "100", rate_per_kg: "2", rate_per_kg_usd: "2" },
      ],
      [
