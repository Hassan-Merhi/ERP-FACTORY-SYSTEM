...
/**
 * Current-schema implementations for reports whose legacy SQL drifted behind
 * table/column renames. Keep this list intentionally small: the registry
 * overrides only these query types while the original phase shards continue to
 * serve every unaffected report.
 */
export const schemaCompatibilityQueryTypes = [
  "container_list",
  "payroll_summary",
...
async function runSchemaCompatibilityReport(ctx: DataQueryContext): Promise<DataQueryResult> {
  ...
  switch (params.queryType) {
    case "container_list": {
      ...
      const tableRows = rows.rows.map((r) => [
        r.container_number,
        r.status,
        r.import_date ? String(r.import_date).slice(0, 10) : "—",
    ...
    case "payroll_summary": {
      ...
      const tableRows = rows.rows.map((r) => {
        ...
        const bale = parseFloat(r.bale_earnings || "0");
      ...
      return {
        ...
        stats: [
          { label: "Workers / Payroll Rows", value: String(tableRows.length) },
          { label: "Base Salary", value: fmt(totalBase) },
          { label: "Bale Earnings", value: fmt(totalBale) },
          { label: "Deductions", value: fmt(totalDeductions) },
          { label: "Net Payroll", value: fmt(totalNet), highlight: "positive" },
        ],
        table: {
          headers: ["Worker", "Period", "Status", "Base", "Bale Earnings", "Deductions", "Net", "Present/Absent"],
          rows: tableRows,
        },
        noData: tableRows.length === 0,
      };
    ...
    case "container_profitability": {
      ...
      if (tableRows.length) {
        const margin = totalSale > 0 ? `${((totalProfit / totalSale) * 100).toFixed(1)}%` : "—";
        tableRows.push(["TOTAL", "", "", "", fmt(totalCost), fmt(totalSale), "", fmt(totalProfit), margin, ""]);
      }
      return {
    ...
    case "worker_productivity": {
      const rows = await db.execute<{
        ...
        SELECT COALESCE(NULLIF(fb.worker_name, ''), 'Unknown') AS full_name,
        ...
        WHERE fb.company_id = ${companyId}
          ...
          AND fb.pressed_at IS NOT NULL
          AND CAST(fb.pressed_at AS text) BETWEEN ${dateFrom} AND ${dateTo}
        GROUP BY COALESCE(NULLIF(fb.worker_name, ''), 'Unknown')
    ...
    case "upcoming_arrivals": {
      ...
      return {
        ...
        subtitle: `Next ${daysAhead} days · ${tableRows.length} container(s) expected`,
        table: {
    ...
    case "container_cost_breakdown": {
      ...
      const stats = [
        ...
        { label: "Import Date", value: container.import_date ? String(container.import_date).slice(0, 10) : "—" },
    ...
    case "stock_item_detail": {
      ...
      const tableRows = inventoryRows.rows.map((r) => {
        ...
        totalQty += qty;
        totalValue += value;
        return [r.location, fmtDec(qty), fmtDec(parseFloat(r.avg_rate || "0")), fmt(value)];
      });
      return {
        queryType: "stock_item_detail",
    ...
    case "supplier_container_history": {
      ...
      return {
        queryType: "supplier_container_history",
        title: `Supplier Containers: ${rows.rows[0]?.supplier || supplierName}`,
        subtitle: `${tableRows.length} container(s)`,
        stats: [
    ...
    default:
      throw new Error(`Schema compatibility report received unsupported query type: ${String(params.queryType)}`);
  }
}

export const schemaCompatibilityReportShard: ReportImplementationShard = {
  name: "schema-compatibility",
  queryTypes: schemaCompatibilityQueryTypes,
  run: runSchemaCompatibilityReport,
};
