...
describe("gitContinuousSnapshots", () => {
  ...
  it("extends the inactivity timeout while a large cursor chain is actively advancing", () => {
    ...
    try {
      ...
      now = 1_060;
      const second = readGitContinuousSnapshot<{ id: number }, Record<string, never>, Record<string, never>>({
        scope: "git:slow-active-chain",
        cursor: first.nextCursor!,
        limit: 1,
      });
      ...
      now = 1_120;
      const third = readGitContinuousSnapshot<{ id: number }, Record<string, never>, Record<string, never>>({
        scope: "git:slow-active-chain",
