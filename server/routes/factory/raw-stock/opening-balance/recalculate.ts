...
export function registerRawStockRecalculateUsedRoutes(app: Express) {
  ...
  app.post("/api/factory/raw-stock/recalculate-used", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      type RecalcChange = {
        ...
        receivedKg: string;
        oldUsedKg: string;
        correctedUsedKg: string;
        differenceKg: string;
        validSourceCount: number;
        validSourceWeightKg: string;
        excludedDeletedSourceCount: number;
        excludedDeletedSourceWeightKg: string;
      };
      ...
      let updated = 0;
      const appliedChanges: RecalcChange[] = [];
      ...
      await db.transaction(async (tx) => {
        for (const c of changes) {
          ...
          await tx
            ...
            .set({ usedKg: correctedUsedKg.toFixed(3) })
            .where(eq(factoryRawStock.id, c.rawStockId));
