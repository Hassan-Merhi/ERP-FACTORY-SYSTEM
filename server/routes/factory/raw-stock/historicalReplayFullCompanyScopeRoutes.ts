    ...
    try {
      ...
      if (!impact || impact.allSafetyGatesPassed !== true) {
        return res.status(409).json({
          message: "Historical Replay cannot prepare until every company-wide safety gate passes.",
      ...
      if (unsafeSuppliers.length > 0) {
        return res.status(409).json({
          message: "Historical Replay cannot prepare while any company supplier timeline requires review.",
