  ...
  app.post("/api/factory/staff-tracking/bulk", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      for (const raw of records) {
        ...
        if (!Number.isInteger(personId) || personId <= 0 || (page === "attendance" && !STATUSES.has(requestedStatus))) {
      ...
      if (finalize) {
        ...
        // Each linked worker displays the team's shared production total, while
        // the client summary de-duplicates the link so factory totals stay exact.
        for (const link of activeProductionLinks) {
