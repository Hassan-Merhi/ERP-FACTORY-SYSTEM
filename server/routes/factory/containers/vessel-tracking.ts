...
import { getAisHealthSnapshot } from "../../../services/ais/aisHealth";
...
/**
 * Wave 4 API boundary: carrier/container tracking remains authoritative for
 * shipment status + ETA; AIS is returned as a separate vessel visibility
 * object so consumers can never mistake an AIS field for carrier truth.
 */
export function registerFactoryContainerVesselTrackingRoutes(app: Express) {
  app.get("/api/factory/containers/:id/vessel-tracking", requireAuth, async (req: Request, res: Response) => {
    ...
    return res.json({
      container: {
        ...
        status: container.trackingLastStatus,
        lastEventDate: container.trackingLastEventDate,
        lastDescription: container.trackingLastDescription,
        eta: container.arrivalDate,
        etaSource: container.trackingProvider || "stored",
        carrier: container.trackingDetectedCarrier,
      },
      vessel: vessel ? {
        ...
        mappingSource: vessel.mappingSource,
        mappingConfidence: vessel.mappingConfidence,
      } : null,
      ais: vessel ? {
        ...
        aisEta: vessel.aisEta,
        lastPositionAt: vessel.lastPositionAt,
        lastUpdateAt: vessel.lastAisUpdateAt,
      } : null,
    });
  });
  ...
  // Operational state only; never exposes the AISStream API key or socket payloads.
  app.get("/api/factory/ais/health", requireAuth, (_req: Request, res: Response) => {
    return res.json(getAisHealthSnapshot());
  });
}
