...
export function registerSnapshotPinnedAccountRoutes(app: Express) {
  ...
  app.get(
    "/api/snapshot-pinned-accounts/:cardKey",
    requireAuth,
    ...
    async (req: import("express").Request, res: import("express").Response) => {
      try {
        ...
        const [row] = await db
          ...
          .onConflictDoUpdate({
            target: [
              snapshotPinnedAccounts.companyId,
              snapshotPinnedAccounts.cardKey,
              snapshotPinnedAccounts.accountId,
            ],
  ...
  app.delete(
    "/api/snapshot-pinned-accounts/:cardKey/:accountId",
    requireAuth,
    async (req: import("express").Request, res: import("express").Response) => {
      try {
        const companyId = req.session.currentCompanyId || req.session.factoryCompanyId;
