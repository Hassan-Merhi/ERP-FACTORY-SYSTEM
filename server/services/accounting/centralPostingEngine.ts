 ...
 *
 * Manual Journal and Payment/Receipt voucher numbers are generated by their HTTP
 * adapters for display only. A retry generates a fresh display number before the
 * idempotency lookup, so including that value would make the same financial
 * request appear different and defeat replay after a lost client response.
 */
export function buildPostingRequestFingerprint(request: CentralPostingRequest): string {
  const sourceType = request.source.sourceType.trim();
  const canonical = {
    companyId: request.voucher.companyId,
    voucherNumber: SERVER_NUMBERED_RETRY_SOURCES.has(sourceType) ? null : request.voucher.voucherNumber.trim(),
