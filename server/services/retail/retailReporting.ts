...
function financialLocationPredicate(locationId?: number): string {
  return locationId ? " AND m.location_id = $4" : "";
}

function inventoryLocationPredicate(locationId?: number): string {
  return locationId ? " AND i.location_id = $2" : "";
}
...
export async function getRetailDashboard(filters: RetailReportFilters) {
  ...
  const slowMovingParams: unknown[] = filters.locationId
    ? [filters.companyId, filters.locationId, slowMovingDays]
    : [filters.companyId, slowMovingDays];
  const slowMovingDaysParam = filters.locationId ? 3 : 2;
  ...
  const [
    ...
    pool.query(
      `WITH catalog AS (
       ...
       SELECT catalog.*, stock.*, low_stock.*, out_stock.*
       FROM catalog CROSS JOIN stock CROSS JOIN low_stock CROSS JOIN out_stock`,
      inventoryParams
    ),
    pool.query(
      `WITH financial_moves AS (
         ...
         WHERE m.company_id = $1
           AND m.created_at >= $2
           AND m.created_at < $3
           AND m.movement_type IN ('sale', 'return', 'cancellation')${financialLocation}
       )
       SELECT
    ...
    pool.query(
      `WITH stock AS (
         ...
         WHERE company_id = $1
           AND movement_type = 'sale'${filters.locationId ? " AND location_id = $2" : ""}
         GROUP BY variant_id
       )
       SELECT stock.*, last_sales.last_sale_at
       FROM stock
       LEFT JOIN last_sales ON last_sales.variant_id = stock.variant_id
       WHERE stock.quantity > 0
         AND (last_sales.last_sale_at IS NULL
              OR last_sales.last_sale_at < NOW() - ($${slowMovingDaysParam}::text || ' days')::interval)
       ORDER BY last_sales.last_sale_at NULLS FIRST, stock.quantity DESC
       LIMIT ${limit}`,
      slowMovingParams
    ),
  ]);
  ...
  const rankings = (rankingsResult.rows[0]?.rankings ?? {
    products: [],
    brands: [],
    sizes: [],
    locations: [],
...
export async function runRetailAudit(companyId: number): Promise<{
  ...
  const [
    ...
    pool.query(
      `SELECT
       ...
       HAVING COUNT(m.id) <> 1
          OR ABS(COALESCE(SUM(m.quantity_delta), 0) + si.quantity) > 0.000001
       LIMIT 25`,
      [companyId]
    ),
    pool.query(
      `SELECT
       ...
       HAVING COUNT(m.id) <> 1
          OR ABS(COALESCE(SUM(m.quantity_delta), 0) - ri.quantity) > 0.000001
       LIMIT 25`,
      [companyId]
    ),
    pool.query(
      `SELECT
       ...
       HAVING COUNT(*) <> 2
          ...
          OR ABS(
            COALESCE(SUM(CASE WHEN quantity_delta < 0 THEN -quantity_delta ELSE 0 END), 0)
            - COALESCE(SUM(CASE WHEN quantity_delta > 0 THEN quantity_delta ELSE 0 END), 0)
          ) > 0.000001
       LIMIT 25`,
      [companyId]
    ...
    pool.query(
      `WITH latest AS (
       ...
       FROM retail_variant_inventory i
       JOIN latest
         ON latest.variant_id = i.variant_id
        AND latest.location_id = i.location_id
       WHERE i.company_id = $1
  ...
  add(
    ...
    "warning",
    productsWithoutVariants.rows,
    "Active products exist without an active valid variant."
  );
  add(
    "duplicate_pos_deductions",
    "error",
    saleDeductions.rows,
    "POS sale items do not reconcile one-to-one to their stock deduction."
  );
  add(
    "return_stock_restoration",
    "error",
    returnRestoration.rows,
    "Retail returns do not reconcile to exact stock restoration movements."
  );
  add(
    "transfer_conservation",
    "error",
    transferPairs.rows,
    "Retail transfers do not conserve quantity between source and destination."
  );
  add(
    "incorrect_inventory_totals",
    "error",
    inventoryTotals.rows,
    "Current inventory does not match the latest movement quantity."
  );
  add(
