          ...
          {payload.map((item, index) => {
            ...
            return (
              <div
                key={item.dataKey}
                className={cn(
                  "flex min-w-0 w-full flex-wrap items-stretch gap-2 [&>svg]:h-2.5 [&>svg]:w-2.5 [&>svg]:text-muted-foreground",
              ...
              >
                {formatter && item?.value !== undefined && item.name ? (
                  ...
                  <>
                    ...
                    <div
                      className={cn(
                        "flex min-w-0 flex-1 gap-3 justify-between leading-none",
...
>(({ className, hideIcon = false, payload, verticalAlign = "bottom", nameKey }, ref) => {
  ...
  return (
    <div
      ref={ref}
      className={cn(
        "flex min-w-0 flex-wrap items-center justify-center gap-x-4 gap-y-2 text-center",
